"""
evaluate.py — Evaluación completa del modelo en test (Experimento 1)

Genera:
  - Macro F1-score, accuracy, precision, recall por clase
  - Matriz de confusión (figura)
  - Ejemplos mal clasificados (figura)
"""
...
def evaluate_on_test(model, test_loader, device):
    """
    Corre el modelo sobre test_loader y recoge predicciones,
    etiquetas reales, imágenes y probabilidades.

    Returns
    -------
    preds, labels       : listas de enteros
    all_images          : tensor con todas las imágenes (CPU, desnormalizadas)
    all_probs           : tensor con softmax de salidas
    """
    model.eval()
